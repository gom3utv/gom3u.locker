 ...
 * Handles Firestore CRUD operations & Firebase Storage Image Uploads.
 */
...
const DOM = {
    ...
    // Form Inputs & Hidden Storage Refs
    playlistId: document.getElementById('playlistId'),
    existingThumbnailUrl: document.getElementById('existingThumbnailUrl'),
    existingThumbnailPath: document.getElementById('existingThumbnailPath'),
...
function initEvents() {
    ...
    if (DOM.thumbnailInput) {
        DOM.thumbnailInput.addEventListener('change', handleFileSelect);
    }

    if (DOM.removeImageBtn) {
        DOM.removeImageBtn.addEventListener('click', resetImageSelection);
    }
...
function renderPlaylistsTable(playlists) {
            ...
            <tr>
                ...
                <td>
                    <div class="action-buttons">
                        ...
                        <button class="btn btn-sm btn-outline-danger delete-btn" data-id="${p.id}" data-title="${escapeHtml(p.title)}" data-path="${p.thumbnailPath || ''}">Delete</button>
                    </div>
                </td>
            </tr>
        `;
    ...
    document.querySelectorAll('.delete-btn').forEach(btn => {
        btn.addEventListener('click', (e) => handleDeletePlaylist(
            e.target.dataset.id, 
...
function openModal() {
    ...
    DOM.existingThumbnailUrl.value = '';
    DOM.existingThumbnailPath.value = '';
    resetImageSelection();
...
/**
 * Opens Modal for Editing Playlist
 */
async function openModalForEdit(id) {
    try {
        ...
        DOM.existingThumbnailUrl.value = data.thumbnailUrl || '';
        DOM.existingThumbnailPath.value = data.thumbnailPath || '';

        resetImageSelection();

        if (data.thumbnailUrl) {
            DOM.imagePreview.src = data.thumbnailUrl;
            DOM.imagePreviewContainer.style.display = 'flex';
        }
...
async function uploadThumbnailFile(file) {
    ...
    const fileName = `thumbnails/thumb_${Date.now()}_${Math.random().toString(36).substring(2, 8)}.${fileExtension}`;
...
/**
 * Handles Form Submission (Uploads image if selected, updates Firestore)
 */
async function handleSavePlaylist(e) {
    ...
    try {
        let thumbnailUrl = DOM.existingThumbnailUrl.value;
        let thumbnailPath = DOM.existingThumbnailPath.value;

        // If a new image file is chosen, upload it to Storage
        if (selectedFile) {
            const uploadResult = await uploadThumbnailFile(selectedFile);
            
            // Delete old file from Storage if updating
            if (thumbnailPath) {
                deleteStorageFile(thumbnailPath);
            }
...
/**
 * Handles Delete Playlist and its associated Storage Image
 */
async function handleDeletePlaylist(id, title, thumbnailPath) {
